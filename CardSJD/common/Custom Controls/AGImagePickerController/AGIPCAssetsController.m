...
- (id)initWithImagePickerController:(AGImagePickerController *)imagePickerController andAssetsGroup:(ALAssetsGroup *)assetsGroup
{
    ...
    if (self)
    {
        ...
        [self.imagePickerController addObserver:self forKeyPath:@"selectAll" options:NSKeyValueObservingOptionNew context:nil];
        
        [self.imagePickerController addObserver:self forKeyPath:@"finished" options:NSKeyValueObservingOptionNew context:nil];
...
{
    if([keyPath isEqualToString:@"selectAll"])
    {
        for(int i=(int)self.assets.count-1;i>=0;i--)
        {
            AGIPCGridItem *obj = self.assets[i];
            obj.selected = self.imagePickerController.selectAll;
    ...
    {
        if(self.imagePickerController.finished)
        {
...
- (void)dealloc
{
    [self.imagePickerController removeObserver:self forKeyPath:@"selectAll"];
    [self.imagePickerController removeObserver:self forKeyPath:@"finished"];
    self.imagePickerController.selectAll = false;
...
- (void)changeSelectionInformation
{
    ...
    self.imagePickerController.selectCount = [AGIPCGridItem numberOfSelections];
